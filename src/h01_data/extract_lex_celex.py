...
VOWELS = set("IE{VQU@i#$u312456789cq0~iI1!eE2@aAoO3#4$6^uU7&5+8*9(0)<>[]{}") # pylint: disable=line-too-long
...
def get_celex_monos(path_base, language, lemma):
...
def get_celex_class(path_base, language, lemma):
...
def get_celex_freq(path_base, language, lemma):
...
def filter_monos(lines, path, language, lemma):
    monos = get_celex_monos(path, language, lemma)
...
def build_celex_corpus(path, language, lemma, mono):
    ...
    if mono:
        lines = filter_monos(lines, path, language, lemma)

    freqs = get_celex_freq(path, language, lemma)
    gram = get_celex_class(path, language, lemma)
