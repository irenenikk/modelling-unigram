...
class Adaptor:
    def __init__(self, a, b, alphabet, token_dataset):
        ...
        self.tokens = self.build_tokens(token_dataset)
        print('Token data length', len(self.tokens))
    
    def build_tokens(self, token_dataset):
        tokens = []
    ...
    def get_token_probability(self, generator, token):
        ...
        i = len(self.tokens)
        return (customers_in_tables_with_label - self.tables_with_word_label[token]*self.a + (self.total_tables*self.a + self.b)*generator_prob)/(i+b)

    def get_word_idx(self, word):
        # this is a duplicate of a method for a dataloader
        # TODO: use a dataloader instead of the vanilla dataset in this class
    ...
    def get_generator_word_probability(self, generator, word):
        word_char_indices = self.get_word_idx(word)
